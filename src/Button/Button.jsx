...
function Button(){
  ...
  { id: 47, name: "MLS Cup", year: 2025, team: "Inter Miami" }
    ...
    const handleClick = () => { setShowLists(!showLists); }
    
    return(
        <div className={styles.container}>
            <button onClick = {handleClick} className={styles.button}>Click Here</button>
            {/* Conditional Rendering: Only show these if showLists is true */}
            {showLists && (
                <>
                    <Awards awards={messiAwards} category="Awards" />
                    <Awards awards={messiSilverware} category="Silverware" />
                </>
            )}
        </div>
    );
